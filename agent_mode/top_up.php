...
<div class="container mt-3">
    ...
    <h4 class="w-50 m-auto my-4">Current Balance: Rp<?php echo number_format($earning_balance, 2, '.', ',');?></h4>
...
if (isset ($_POST['edit_amount'])) {
    ...
    if ($added_amount == '') {
        ...
        $formatted_added_amount = number_format($added_amount, 2, '.', ',');
        $formatted_new_amount = number_format($new_amount, 2, '.', ',');
        $formatted_new_paid = number_format($new_paid, 2, '.', ',');
        $formatted_earning_total = number_format($earning_total, 2, '.', ',');

        $trans_statement = "$current_date^ You withdrew Rp$formatted_added_amount from your QuickCart Wallet Account^ $formatted_new_amount^ $formatted_new_paid^ $formatted_earning_total";
        ...
        if ($result_query) {
            echo "<script>alert('Wallet has been updated successfully. Updated balance = Rp" . number_format($new_amount, 2, '.', ',') . "'); window.location.href = 'profile_page.php?agent_id=" . $agent_id . "&wallet_history';</script>";
        }
    }
}
?>